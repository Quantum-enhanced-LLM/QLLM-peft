...
class LoraLayer(BaseTunerLayer):
    ...
    adapter_layer_names = ("lora_A", "lora_B", "lora_embedding_A", "lora_embedding_B",
                           ...
                           "lora_qpeft_NW1", "lora_qpeft_NW2",
                           "lora_qpeft_NW3", "lora_qpeft_NW4",
                           "lora_qpeft_CW", 
                           "lora_qpeft_QW",)
    ...
    def __init__(self, base_layer: nn.Module, ephemeral_gpu_offload: bool = False, **kwargs) -> None:
        ...
        if not self.use_qpeft: 
            ...
            if qpeft_arch == 'ABC': # MPO + MLP + Quantum 
                ...
                self.lora_qpeft_MLP_B = nn.ModuleDict({})
                self.lora_qpeft_CW = nn.ModuleDict({})
            elif qpeft_arch == 'C': # Quantum only
                self.lora_qpeft_Q = nn.ModuleDict({})
                self.lora_qpeft_MLP_A = nn.ModuleDict({})
                self.lora_qpeft_MLP_B = nn.ModuleDict({})
                self.lora_qpeft_QW = nn.ModuleDict({})
                ...
                self.lora_qpeft_MLP_B = nn.ModuleDict({})
                self.lora_qpeft_CW = nn.ModuleDict({})
                self.lora_qpeft_NW1 = nn.ModuleDict({})
                self.lora_qpeft_NW2 = nn.ModuleDict({})
            elif qpeft_arch == 'BE':    
                self.lora_qpeft_MLP_A = nn.ModuleDict({})
                self.lora_qpeft_MLP_B = nn.ModuleDict({})
                self.lora_qpeft_CW = nn.ModuleDict({})
                self.lora_qpeft_NW1 = nn.ModuleDict({})
                self.lora_qpeft_NW2 = nn.ModuleDict({})
                self.lora_qpeft_NW3 = nn.ModuleDict({})
            elif qpeft_arch == 'BF':    
                self.lora_qpeft_MLP_A = nn.ModuleDict({})
                self.lora_qpeft_MLP_B = nn.ModuleDict({})
                self.lora_qpeft_CW = nn.ModuleDict({})
                ...
                self.lora_qpeft_NW2 = nn.ModuleDict({})
                self.lora_qpeft_NW3 = nn.ModuleDict({})
                self.lora_qpeft_NW4 = nn.ModuleDict({})
            else:
                raise ValueError(f"Invalid qpeft_arch {self.qpeft_arch}.")

            if 'C' in qpeft_arch:
                self.n_qlayers = kwargs.get('qpeft_qcircuit_layers')
    ...
    def _update_layer_qpeft(self, adapter_name, r):        
        ...
        if 'C' in self.qpeft_arch:
            ...
            if self.qpeft_arch == 'C': # C only
                # we need to build MLP layers
                self.lora_qpeft_MLP_A[adapter_name] = nn.Linear(self.in_features, r, bias=False)
                self.lora_qpeft_MLP_B[adapter_name] = nn.Linear(r, self.out_features, bias=False)
                nn.init.kaiming_uniform_(self.lora_qpeft_MLP_A[adapter_name].weight)
                nn.init.zeros_(self.lora_qpeft_MLP_B[adapter_name].weight)

        if 'D' in self.qpeft_arch:
            self.lora_qpeft_NW1[adapter_name] = nn.Linear(r, r, bias=False)
            self.lora_qpeft_NW2[adapter_name] = nn.Linear(r, r, bias=False)

            nn.init.kaiming_uniform_(self.lora_qpeft_NW1[adapter_name].weight)
            nn.init.zeros_(self.lora_qpeft_NW2[adapter_name].weight)
        
        if 'E' in self.qpeft_arch:
            self.lora_qpeft_NW1[adapter_name] = nn.Linear(r, r, bias=False)
            self.lora_qpeft_NW2[adapter_name] = nn.Linear(r, r, bias=False)
            self.lora_qpeft_NW3[adapter_name] = nn.Linear(r, r, bias=False)

            nn.init.kaiming_uniform_(self.lora_qpeft_NW1[adapter_name].weight)
            nn.init.kaiming_uniform_(self.lora_qpeft_NW2[adapter_name].weight)
            nn.init.zeros_(self.lora_qpeft_NW3[adapter_name].weight)
        
        if 'F' in self.qpeft_arch:
            self.lora_qpeft_NW1[adapter_name] = nn.Linear(r, r, bias=False)
            self.lora_qpeft_NW2[adapter_name] = nn.Linear(r, r, bias=False)
            self.lora_qpeft_NW3[adapter_name] = nn.Linear(r, r, bias=False)
            self.lora_qpeft_NW4[adapter_name] = nn.Linear(r, r, bias=False)

            nn.init.kaiming_uniform_(self.lora_qpeft_NW1[adapter_name].weight)
            nn.init.kaiming_uniform_(self.lora_qpeft_NW2[adapter_name].weight)
            nn.init.kaiming_uniform_(self.lora_qpeft_NW3[adapter_name].weight)
            nn.init.zeros_(self.lora_qpeft_NW4[adapter_name].weight)
            ...
            for active_adapter in self.active_adapters:
                ...
                if not self.use_qpeft:
                    ...
                    if self.qpeft_arch == 'ABC':
                        ...
                        quantum_out = lora_Q(lora_ca_output)
                        output_qw = lora_QW(quantum_out.to(lora_QW.weight.dtype))
                        output_cw = lora_CW(lora_ca_output)
                        lora_cb_in = output_qw + output_cw
                        ...
                        quantum_out = lora_Q(lora_ca_output)
                        output_qw = lora_QW(quantum_out.to(lora_QW.weight.dtype))
                        lora_cb_in = output_qw
                        result = result + lora_CB(lora_cb_in) * scaling
                    elif self.qpeft_arch == 'BD':
                        lora_CA = self.lora_qpeft_MLP_A[active_adapter]
                        lora_CB = self.lora_qpeft_MLP_B[active_adapter]
                        lora_CW = self.lora_qpeft_CW[active_adapter]

                        lora_NW1 = self.lora_qpeft_NW1[active_adapter]
                        lora_NW2 = self.lora_qpeft_NW2[active_adapter]
                        silu = nn.SiLU()

                        lora_ca_output = lora_CA(dropout(x))


                        output_qw = lora_NW2(silu(lora_NW1(lora_ca_output)))
                        output_cw = lora_CW(lora_ca_output)
                        lora_cb_in = output_qw + output_cw
                        
                        result = result + lora_CB(lora_cb_in) * scaling
                    elif self.qpeft_arch == 'BE':                        
                        lora_CA = self.lora_qpeft_MLP_A[active_adapter]
                        lora_CB = self.lora_qpeft_MLP_B[active_adapter]
                        lora_CW = self.lora_qpeft_CW[active_adapter]
                        ...
                        lora_NW1 = self.lora_qpeft_NW1[active_adapter]
                        lora_NW2 = self.lora_qpeft_NW2[active_adapter]
                        lora_NW3 = self.lora_qpeft_NW3[active_adapter]
                        silu = nn.SiLU()

                        lora_ca_output = lora_CA(dropout(x))

                        output_qw = lora_NW3(silu(lora_NW2(silu(lora_NW1(lora_ca_output)))))
                        output_cw = lora_CW(lora_ca_output)
                        lora_cb_in = output_qw + output_cw
                        
                        result = result + lora_CB(lora_cb_in) * scaling
                    elif self.qpeft_arch == 'BF':
                        lora_CA = self.lora_qpeft_MLP_A[active_adapter]
                        lora_CB = self.lora_qpeft_MLP_B[active_adapter]
                        lora_CW = self.lora_qpeft_CW[active_adapter]
                        ...
                        lora_NW2 = self.lora_qpeft_NW2[active_adapter]
                        lora_NW3 = self.lora_qpeft_NW3[active_adapter]
                        lora_NW4 = self.lora_qpeft_NW4[active_adapter]
                        silu = nn.SiLU()

                        lora_ca_output = lora_CA(dropout(x))

                        output_qw = lora_NW4(silu(lora_NW3(silu(lora_NW2(silu(lora_NW1(lora_ca_output)))))))
                        output_cw = lora_CW(lora_ca_output)
                        lora_cb_in = output_qw + output_cw
